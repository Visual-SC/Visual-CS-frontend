...
const OrderCreated: React.FC = () => {
  ...
  return (
    <section className="order-created z-20 fixed flex flex-col gap-2 w-160 h-auto p-8 top-16 bg-white rounded-xl">
      ...
      <article className="flex flex-col w-75 mx-auto">
        <img src="/pedido-confirmado-redson-coffe.png" alt="Orden creada" className="w-full" />
        <div className="flex flex-col items-center gap-1 mt-2 text-h4-20 font-semibold">
          <p>Mesa: {order.numero_mesa}</p>
          ...
          <ul className="flex flex-col  justify-between w-full mt-2">
              {
                order.items.map((item) => {
                  return(
                    <li className="grid grid-cols-[200px_1fr] justify-between h-9 w-full mt-2 items-center">
